...
class _WeightStepperButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Material(
    ...
    child: InkWell(
      onTap: onPressed,
      borderRadius: BorderRadius.circular(16),
      child: Center(child: Icon(icon, color: AppColors.textPrimary, size: 22)),
    ),
  );
}
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(
        colors: [
          ...
          AppColors.lilac.withValues(alpha: .12),
        ],
      ),
...
class _WeightChartPainter extends CustomPainter {
  ...
  TextPainter _labelPainter(String text, {Color? color}) => TextPainter(
    text: TextSpan(
      text: text,
      style: TextStyle(color: color ?? AppColors.textSecondary, fontSize: 10),
    ),
